...
# Create Model
model = ModelV1(input_dim=X_train.shape[1], hidden_dim=32).to(device)
...
MODEL_PATH = Path("../NN_hadr_deeper")
...
list_test_acc = []
list_chi_squared = []
df = pd.DataFrame(columns=['epoch', 'train_loss', 'test_loss', 'train_acc', 'test_acc', 'chi_squared'])
# Training Loop
for epoch in tqdm(range(EPOCHS)):
    ...
    if epoch % 1 == 0:
        ...
        df = df._append({'epoch': epoch, 'train_loss': loss.item(), 'test_loss': test_loss.item(), 'train_acc': acc, 'test_acc': test_acc, 'chi_squared': chi_squared_3d(y_test, test_pred, X_test)}, ignore_index=True)
...
PLOT_PATH.mkdir(parents=True,
                exist_ok=True)
PLOT_NAME = "nn_hadrons_metrics_deeper.png"
...
# Write dataframe to csv file and complete if already exists
CSV_PATH = Path('../csv')
CSV_PATH.mkdir(parents=True,
                exist_ok=True)
CSV_NAME = "nn_hadrons_metrics_deeper.csv"
CSV_SAVE_PATH = CSV_PATH / CSV_NAME
