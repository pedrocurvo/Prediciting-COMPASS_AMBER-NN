...
def plt_hist2d(xb_array, y_array, x_bins, y_bins, title, xlabel, ylabel, weights=None, log=True):

    # Calculate the 2D histogram using np.histogram2d()
    hist_gen, x_edges, y_edges = np.histogram2d(xb_array, y_array, bins=[x_bins, y_bins], weights=weights)
...
def plt_hist(X, title, xlabel, ylabel, weights=None, log=True):
